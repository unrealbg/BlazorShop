...
public sealed class E2EApplicationFixture : IAsyncLifetime
{
    ...
    private async Task ExportApplicationLogsAsync(ResourceLoggerService resourceLogger)
    {
        ...
        foreach (var resourceName in new[] { "postgres", "apiservice", "storefront", "adminclient" })
        {
            ...
            {
                lines.AddRange(batch.Select(line => line.ToString()));
            }

            await File.WriteAllLinesAsync(
                Path.Combine(ArtifactDirectory, $"{resourceName}.log"),
                lines);
        }
    }

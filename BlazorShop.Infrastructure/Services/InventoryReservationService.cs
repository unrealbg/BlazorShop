            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return await executionStrategy.ExecuteAsync(async () =>
            {
                ...
                if (reservations.Count == 0)
                {
                            ...
                            && reservationStatus == InventoryReservationStatus.Released))
                    {
                        return await RollBackAsync(
                            db,
                            transaction,
                            new InventoryTransitionResult(InventoryTransitionOutcome.AlreadyApplied),
                            cancellationToken);
                    }
                ...
                foreach (var reservation in reservedItems)
                {
                    ...
                    {
                        // Legacy/manual deletes cannot be physically restored, but must not strand the order.
